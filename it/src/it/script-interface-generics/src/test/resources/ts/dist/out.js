// dist/StringListProcessor_Builtins.mjs
function log(message) {
}

// dist/StringListProcessor.js
function toUpperCase(items) {
  log("Converting " + items.length + " items to uppercase");
  return items.map((item) => item.toUpperCase());
}
function filterContaining(items, filter) {
  log("Filtering items containing '" + filter + "'");
  return items.filter((item) => item.includes(filter));
}
function joinStrings(items, delimiter) {
